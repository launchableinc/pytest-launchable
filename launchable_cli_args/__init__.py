...
class CLIArgs:
    ...
    def fill_and_validate(self, data:dict):
        ...
        if self.launchable_token is None:
            error_counter.record("environment variable LAUNCHABLE_TOKEN is not defined. please set it to enable Launchable features.")
        self.build_id = data.get("build-id", None)
        self.cached_build_id = None
        if self.build_id is None:
            error_counter.record("build_id is not specified")
        self.record_build.fill_and_validate(data.get("record-build", None), error_counter)
        self.record_session.fill_and_validate(data.get("record-session", None), error_counter)
        self.subset.fill_and_validate(data.get("subset", None), error_counter)
        self.record_tests.fill_and_validate(data.get("record-tests", None), error_counter)
    ...
    def check_mandatory_field(self, data:dict, key:str, verifier:callable, error_counter:ErrorCounter):
        ...
        if value is None:
            error_counter.record("object from line %d: key %s is not found" % (line_info["__begin__"], key))
            return None
        else:
            msg = verifier(value)
            if msg is not None:
                error_counter.record("line %d: @%s: %s" % (line_info[key], key, msg))
                return None
            else:
                return value
    ...
    def check_int_field(self, data:dict, key:str, default_value:int, error_counter:ErrorCounter):
        ...
        if value is None:
            ...
            try:
                ...
                error_counter.record("line %d attribute %s: %s is not an integer" % (line_info[key], key, str(value)))
                return None
...
def git_rev_parse(dir):
	h = subprocess.run(("git", "rev-parse", "--short", "HEAD"), cwd=dir, stdout=subprocess.PIPE, text=True).stdout
